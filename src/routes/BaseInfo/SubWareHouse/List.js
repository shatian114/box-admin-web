 ...
 * @Last Modified time: 2018-06-11 17:51:01
 * @Description: 用户管理列表
 */
...
const dicType = 'SubWareHouse';
const url = 'dic';

const routerUrl = '/warHouseSettting/subWareHouse';
...
export default class MyList extends Component {
  ...
  render() {
    ...
    const { WareHouseType, WareHouse } = base;
    ...
    const columns = [
      ...
      },
      {
        ...
        render: text => {
          if (Array.isArray(WareHouse)) {
            const temp = WareHouse.find(item => item.dic_code === text);
      ...
      },
      {
        ...
        render: text => {
          if (Array.isArray(WareHouseType)) {
            const temp = WareHouseType.find(item => item.dic_code === text);
            if (temp) return `${temp.dic_name}(${text})`;
            return text;
          }
        },
              ...
              <Col md={8} sm={24}>
                <FormItem label="总库编号">
                  {getFieldDecorator('parentId', {
                    initialValue: this.props.list.queryMap.parentId,
                  })(<Input placeholder="请输入" />)}
                </FormItem>
              </Col>
              <Col md={12} sm={24}>
