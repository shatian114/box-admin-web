  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        const { dispatch } = this.props;
        if (this.props.base.info.moduleId) {
          dispatch({
            type: 'base/fetch',
            payload: {
              ...values,
    ...
    return (
      <Spin size="large" spinning={loading}>
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormItem {...formItemLayout} hasFeedback label="模块名称">
            {getFieldDecorator('moduleName', {
              initialValue: info.moduleName || newInfo.moduleName,
              rules: [
                {
                  required: true,
                  message: '模块名称不能缺失!',
                },
                { max: 255, message: '模块名称必须小于255位!' },
              ],
            })(<Input placeholder="请输入" />)}
          </FormItem>
          <FormItem {...formItemLayout} hasFeedback label="模块描述">
            {getFieldDecorator('moduleDesc', {
              initialValue: info.moduleDesc || newInfo.moduleDesc,
              rules: [
                {
                  required: true,
                  message: '模块描述不能缺失!',
                },
                { max: 255, message: '模块描述必须小于255位!' },
              ],
            })(<Input placeholder="请输入" />)}
          </FormItem>
