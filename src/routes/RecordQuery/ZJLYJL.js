 ...
 * @Last Modified time: 2018-06-12 09:04:04
 * @Description: 用户管理列表
 */
...
const routerUrl = '/ZJLYJL';

@connect(({ base }) => ({ base }))
@Form.create()
@List.create()
@Operate.create(routerUrl)
export default class ZJLYJL extends Component {
  state = {
        ...
        <Card bordered={false} style={{ marginBottom: 24 }} hoverable>
          <Form onSubmit={this.handleSearch} layout="inline">
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
              ...
              <Col md={8} sm={24}>
                <FormItem label="领用数量(起始)">
                  {getFieldDecorator('start_get_count', {
                    initialValue: this.props.list.queryMap.start_get_count,
                  })(<InputNumber placeholder="请输入" />)}
                </FormItem>
              </Col>
              <Col md={8} sm={24}>
                <FormItem label="领用数量(结束)">
                  {getFieldDecorator('end_get_count', {
                    initialValue: this.props.list.queryMap.end_get_count,
                  })(<InputNumber placeholder="请输入" />)}
                </FormItem>
              </Col>
              <Col md={8} sm={24}>
