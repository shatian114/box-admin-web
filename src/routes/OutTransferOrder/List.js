 ...
 * @Last Modified time: 2018-06-11 17:54:15
 * @Description: 用户管理列表
 */
...
export default class MyIndex extends Component {
  ...
  handleSearch = e => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      let temp = {};
      if (!isEmpty(values.startDateOut))
        temp = {
          ...temp,
          startDateOut: values.startDateOut.format(DateFormat),
        };
      if (!isEmpty(values.endDateOut))
        temp = {
          ...temp,
          endDateOut: values.endDateOut.format(DateFormat),
        };
      if (!isEmpty(values.startDateIn))
        temp = {
          ...temp,
          startDateIn: values.startDateIn.format(DateFormat),
        };
      if (!isEmpty(values.endDateIn))
        temp = {
          ...temp,
          endDateIn: values.endDateIn.format(DateFormat),
        };
      setList({
        current: 1,
  ...
  render() {
    ...
    const listConfig = {
      url: '/api/query/queryOutTransferList', // 必填,请求url
