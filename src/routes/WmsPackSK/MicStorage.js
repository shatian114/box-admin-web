...
export default class MicStorage extends React.PureComponent {
  ...
  verifyCode = (item, lotCode) => {
    if (lotCode === null || lotCode === undefined) {
      this.msg = '批号为空';
      return true;
    } else if (this.state.lot.size > 0 && !this.state.lot.has(lotCode)) {
      this.msg = '当前只能输入一批';
      return true;
    }
  ...
  render() {
    const { info, TLocation, newInfo } = this.props.base;
            ...
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
              <Col md={12} sm={24}>
                ...
                <Col md={24}>
                    ...
                    <Col md={12} sm={24}>
                      <FormItem label="货位">
                        {getFieldDecorator('location', {
                          initialValue: info.location || newInfo.location,
                          rules: [
                            {
                              max: 64,
                              message: '载具规格必须小于64位!',
                  ...
                  <div style={{ fontSize: 50, color: '#f5222d' }}>当前批次:</div>
                  <div style={{ fontSize: 50, color: '#f5222d' }}>
                    {this.state.lot.size > 0 ? Array.from(this.state.lot)[0] : '暂无'}
                  </div>
                </Col>
              </Col>
