...
const { Option } = Select;
//const routerUrl = cache.keysMenu.T1wuyeqftj;
const routerUrl ='/T1wuyeqftj';
const url = 'T1wuyeqftj';
const rowKey = 't_1wuyeqftj_id';
const DateFormat = 'YYYY-MM-DD';

@connect(({ base }) => ({ base }))
@Form.create()
@List.create()
export default class T1wuyeqftjList extends Component {
  state = {
...
temp = {
    ...
    const columns = [
      {
       ...
       {  title: '',   dataIndex: 't_1wuyeqftj_id',     width: 150,     sorter: false,      },
 ...
 {  title: '',   dataIndex: 'qfs',     width: 150,     sorter: false,      },
 {  title: '',   dataIndex: 'qfje',     width: 150,     sorter: false,      },
 ...
 {  title: '创建时间',   dataIndex: 'create_date',     width: 150,     sorter: false,      },
    ...
    const listConfig = {
      url: '/api/T1wuyeqftj/queryT1wuyeqftjList', // 必填,请求url
        ...
        <Card bordered={false} style={{ marginBottom: 24 }} hoverable>
          <Form onSubmit={this.handleSearch}>
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
              <Col {...formItemGrid}>  <FormItem {...formItemLayout} label=''>{getFieldDecorator('t_1wuyeqftj_id',{initialValue: this.props.list.queryMap.t_1wuyeqftj_id, })(<Input placeholder='请输入' />)} </FormItem> </Col>
...
<Col {...formItemGrid}>  <FormItem {...formItemLayout} label='(结束)'>{getFieldDecorator('end_zhs',{initialValue: this.props.list.queryMap.end_zhs  ? moment(this.props.list.queryMap.end_zhs): null, })
 (<InputNumber  placeholder='请输入' />)} </FormItem> </Col>
<Col {...formItemGrid}>  <FormItem {...formItemLayout} label='(起始)'>{getFieldDecorator('start_qfs',{initialValue: this.props.list.queryMap.start_qfs  ? moment(this.props.list.queryMap.start_qfs): null, })
 (<InputNumber  placeholder='请输入' />)} </FormItem> </Col>
<Col {...formItemGrid}>  <FormItem {...formItemLayout} label='(结束)'>{getFieldDecorator('end_qfs',{initialValue: this.props.list.queryMap.end_qfs  ? moment(this.props.list.queryMap.end_qfs): null, })
 (<InputNumber  placeholder='请输入' />)} </FormItem> </Col>
<Col {...formItemGrid}>  <FormItem {...formItemLayout} label='(起始)'>{getFieldDecorator('start_qfje',{initialValue: this.props.list.queryMap.start_qfje  ? moment(this.props.list.queryMap.start_qfje): null, })
 (<InputNumber  placeholder='请输入' />)} </FormItem> </Col>
<Col {...formItemGrid}>  <FormItem {...formItemLayout} label='(结束)'>{getFieldDecorator('end_qfje',{initialValue: this.props.list.queryMap.end_qfje  ? moment(this.props.list.queryMap.end_qfje): null, })
 (<InputNumber  placeholder='请输入' />)} </FormItem> </Col>
