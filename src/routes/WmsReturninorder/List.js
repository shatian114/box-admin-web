 ...
 * @Last Modified time: 2018-06-13 20:32:18
 * @Description: 用户管理列表
 */
...
const { Option } = Select;
const routerUrl = '/WmsReturninorder';
...
export default class WmsReturninorderList extends Component {
  ...
  handleSearch = e => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      let temp = {};
      if (!isEmpty(values.start_inware_date))
        temp = {
          ...temp,
          start_inware_date: values.start_inware_date.format(DateFormat),
        };
      if (!isEmpty(values.end_inware_date))
        temp = {
          ...temp,
          end_inware_date: values.end_inware_date.format(DateFormat),
        };
  ...
  render() {
    ...
    const columns = [
      ...
      { title: '运输车辆', dataIndex: 'vehicle_number', width: 150, sorter: false },
      {
        title: '入库仓库',
      ...
      { title: '入库时间', dataIndex: 'inware_date', width: 150, sorter: false },
      { title: '创建时间', dataIndex: 'create_date', width: 150, sorter: false },
    ];

    const listConfig = {
      url: '/api/WmsReturninorder/queryWmsReturninorderList', // 必填,请求url
        ...
        <Card bordered={false} style={{ marginBottom: 24 }} hoverable>
          <Form onSubmit={this.handleSearch} layout="inline">
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
              ...
              <Col md={8} sm={24}>
                <FormItem label="入库时间(起始)">
                  {getFieldDecorator('start_inware_date', {
                    initialValue: this.props.list.queryMap.start_inware_date
                      ? moment(this.props.list.queryMap.start_inware_date)
                      : null,
              ...
              <Col md={8} sm={24}>
                <FormItem label="入库时间(结束)">
                  {getFieldDecorator('end_inware_date', {
                    initialValue: this.props.list.queryMap.end_inware_date
                      ? moment(this.props.list.queryMap.start_inware_date)
                      : null,
                  })(<DatePicker format={DateFormat} placeholder="请输入" />)}
                </FormItem>
