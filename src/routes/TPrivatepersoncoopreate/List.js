...
const { Option } = Select;
//const routerUrl = cache.keysMenu.TPrivatepersoncoopreate;
const routerUrl ='/TPrivatepersoncoopreate';
const url = 'TPrivatepersoncoopreate';
const rowKey = 't_privatepersoncoopreate_id';
const DateFormat = 'YYYY-MM-DD';

@connect(({ base }) => ({ base }))
@Form.create()
@List.create()
export default class TPrivatepersoncoopreateList extends Component {
  state = {
...
temp = {
  ...
  handleExport = e => {
    ...
    dispatch({
        ...
        payload: {
        filename: 't_privatepersoncoopreate.xls',
    ...
    const columns = [
      {
       ...
       {  title: '',   dataIndex: 't_privatepersoncoopreate_id',     width: 150,     sorter: false,      },
 {  title: '代理id：这个不用系统生成，手工设定，长度是6位数字',   dataIndex: 'uniqueID',     width: 150,     sorter: false,      },
 ...
 {  title: '区域编号',   dataIndex: 'zone',     width: 150,     sorter: false,      },
 ...
 {  title: '创建时间',   dataIndex: 'create_date',     width: 150,     sorter: false,      },
    ...
    const listConfig = {
      url: '/api/TPrivatepersoncoopreate/queryTPrivatepersoncoopreateList', // 必填,请求url
      scroll: { x: 2700, y: this.state.scrollY }, // 可选配置,同antd table
      rowKey, // 必填,行key
      columns, // 必填,行配置
    };
        ...
        <Card bordered={false} style={{ marginBottom: 24 }} hoverable>
          <Form onSubmit={this.handleSearch} layout="inline">
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
              <Col {...formItemGrid}>  <FormItem {...formItemLayout} label=''>{getFieldDecorator('t_privatepersoncoopreate_id',{initialValue: this.props.list.queryMap.t_privatepersoncoopreate_id, })(<Input placeholder='请输入' />)} </FormItem> </Col>
<Col {...formItemGrid}>  <FormItem {...formItemLayout} label='代理id：这个不用系统生成，手工设定，长度是6位数字'>{getFieldDecorator('uniqueID',{initialValue: this.props.list.queryMap.uniqueID, })(<Input placeholder='请输入' />)} </FormItem> </Col>
...
<Col {...formItemGrid}>  <FormItem {...formItemLayout} label='区域编号'>{getFieldDecorator('zone',{initialValue: this.props.list.queryMap.zone, })(<Input placeholder='请输入' />)} </FormItem> </Col>
