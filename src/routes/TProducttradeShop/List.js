...
const { Option } = Select;
//const routerUrl = cache.keysMenu.TProducttradeShop;
const routerUrl ='/TProducttradeShop';
const url = 'TProducttradeShop';
const rowKey = 't_producttrade_shop_id';
const DateFormat = 'YYYY-MM-DD';

@connect(({ base }) => ({ base }))
@Form.create()
@List.create()
export default class TProducttradeShopList extends Component {
  state = {
...
temp = {
  ...
  handleExport = e => {
    ...
    dispatch({
        ...
        payload: {
        filename: 't_producttrade_shop.xls',
    ...
    const columns = [
      {
       ...
       {  title: '',   dataIndex: 't_producttrade_shop_id',     width: 150,     sorter: false,      },
 ...
 {  title: '创建时间',   dataIndex: 'create_date',     width: 150,     sorter: false,      },
    ...
    const listConfig = {
      url: '/api/TProducttradeShop/queryTProducttradeShopList', // 必填,请求url
        ...
        <Card bordered={false} style={{ marginBottom: 24 }} hoverable>
          <Form onSubmit={this.handleSearch}>
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
              <Col {...formItemGrid}>  <FormItem {...formItemLayout} label=''>{getFieldDecorator('t_producttrade_shop_id',{initialValue: this.props.list.queryMap.t_producttrade_shop_id, })(<Input placeholder='请输入' />)} </FormItem> </Col>
