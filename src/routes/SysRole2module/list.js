...
const { Option } = Select;
const routerUrl = '/SysRole2module';
...
@Form.create()
@List.create()
export default class SysRole2moduleList extends Component {
  state = {
  ...
  render() {
    ...
    const columns = [
      ...
      { title: '数据集', dataIndex: 'data_set', width: 150, sorter: false },
    ];

    const listConfig = {
      url: '/api/SysRole2module/querySysRole2moduleList', // 必填,请求url
              ...
              <Col md={8} sm={24}>
                <FormItem label="数据集">
                  {getFieldDecorator('data_set', {
                    initialValue: this.props.list.queryMap.data_set,
                  })(<Input placeholder="请输入" />)}{' '}
                </FormItem>{' '}
              </Col>
