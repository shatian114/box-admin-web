...
{getFieldDecorator('tPrivatepersoncoopreateId', {
 initialValue: info.tPrivatepersoncoopreateId || newInfo.tPrivatepersoncoopreateId,
  rules: [
    {
      required: true,
      message: '不能缺失!',
 ...
 <FormItem {...formItemLayout} hasFeedback label="代理id：这个不用系统生成，手工设定，长度是6位数字">
{getFieldDecorator('uniqueID', {
 initialValue: info.uniqueID ||  newInfo.uniqueID,
  rules: [
    {
      required: true,
      message: '代理id：这个不用系统生成，手工设定，长度是6位数字不能缺失!',
    },{ max: 255,message: '代理id：这个不用系统生成，手工设定，长度是6位数字必须小于255位!',   },
  ],
...
{getFieldDecorator('ip', {
 ...
 <FormItem {...formItemLayout} hasFeedback label="平台提成比例">
{getFieldDecorator('rate', {
 initialValue: info.rate ||  newInfo.rate,
  rules: [
    {
      required: true,
      message: '平台提成比例不能缺失!',
    },{ max: 255,message: '平台提成比例必须小于255位!',   },
  ],
 })(<Input placeholder="请输入" />)}
 </FormItem>
...
{getFieldDecorator('weixin', {
 ...
 <FormItem {...formItemLayout} hasFeedback label="区域编号">
  ...
  rules: [
    {
      required: true,
      message: '区域编号不能缺失!',
    },{ max: 255,message: '区域编号必须小于255位!',   },
  ],
 })(<Input placeholder="请输入" />)}
 </FormItem>
 <FormItem {...formItemLayout} hasFeedback label="结算周期管理:天数">
{getFieldDecorator('clearperoid', {
 initialValue: info.clearperoid ||  newInfo.clearperoid,
  rules: [
    {
      required: true,
      message: '结算周期管理:天数不能缺失!',
    },{ required: true,message: '结算周期管理:天数不能缺失!', },
  ],
 })(<InputNumber min={0} disabled />)}
 </FormItem>
